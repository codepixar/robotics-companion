...
add_action( 'wp_enqueue_scripts', 'robotics_sl_enqueue_scripts' );
function robotics_sl_enqueue_scripts() {

	wp_enqueue_script( 'robotics-post-likes', plugins_url( '../js/post-likes.js', __FILE__ ), array( 'jquery' ), '0.5', false );

	wp_localize_script( 'robotics-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'robotics-core' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_robotics_process_simple_like', 'robotics_process_simple_like' );
add_action( 'wp_ajax_robotics_process_simple_like', 'robotics_process_simple_like' );
function robotics_process_simple_like() {
	// Robotics
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'robotics-core' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	$output = '';
	$nonce = wp_create_nonce( 'simple-likes-nonce' ); // Robotics
	if ( $is_comment == 1 ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'robotics-core' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'robotics-core' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'robotics-core' );
	if ( is_numeric( $like_count ) && $like_count > 0 ) { 
		$number = sl_format_count( $like_count );
	} else {
		$number = $like_text;
	}
	$count = '<span>' . $number .' Likes'.'</span>';
	return $count;
} // get_like_count()

// User Profile List
add_action( 'show_user_profile', 'robotics_show_user_likes' );
add_action( 'edit_user_profile', 'robotics_show_user_likes' );
function robotics_show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'robotics-core' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'robotics-core' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
